...
import { handleTestSize, testConfigurationAtom, setTestConfigurationAtom } from "#root/atoms/test_configuration";
...
function TestConfigBar() {
  const { punctuation, numbers, testSize } = useAtomValue(testConfigurationAtom);
  ...
  return (
    <motion.div
      ...
      variants={variants}
      className={clsx("inline-flex mx-auto items-center bg-surface2 md:gap-2 rounded-md px-2 py-1.5 fullbleed", {
    ...
    >
      <div className="flex justify-center gap-2">
        <ToggleGroup
          type="multiple"
          value={[punctuation ? "punctuation" : "", numbers ? "numbers" : ""]}
          onValueChange={(value) => {
        ...
        >
          <ToggleGroupItem id="punctuation" value="punctuation" aria-label="Toggle punctuation">
            <AtSign />
            <span className="hidden md:block">punctuation</span>
          </ToggleGroupItem>
          <ToggleGroupItem id="numbers" value="numbers" aria-label={"Toggle numbers"}>
            <Hash />
      ...
      <div className="flex justify-center gap-2">
        <ToggleGroup type="single" value={testSize + ""} onValueChange={(value) => setTestSize(parseInt(value))}>
          {sizes.map((s) => (
            <ToggleGroupItem
              key={"test-size-" + s}
              id={"test-size-" + s}
